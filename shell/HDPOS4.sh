...
HDPOS4REDISDATADIR=${HDPOS4REDISDATADIR:-/${DEPLOYDIR}/headingdata/${ENVNAME}/hdpos4_${ENVNAME}/redisdata}
...
start_container() {
    ...
    ID=$(sudo docker run -d \
					     ...
					     --name ${IMAGE}_${ENVNAME} ${DOCKERHUB}/hdpos46/${H4IMAGE}-dist:${VERSION}) >/dev/null 2>&1
}
    ...
    ID=$(sudo docker exec \
						...
						${IMAGE}_${ENVNAME} sh -c '
												sed -i  "
													...
													s%runMode=development%runMode=production%" /opt/heading/tomcat7/webapps/${CH4IMAGE}-web/WEB-INF/classes/hdpos4-web.properties') >/dev/null 2>&1
}
