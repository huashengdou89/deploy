...
start_container() {
    ...
    ID=$(sudo docker run -d \
					     ...
					     --name ${IMAGE}_${ENVNAME} ${DOCKERHUB}/hdpos46/${IMAGE}:${VERSION}) >/dev/null 2>&1
}
...
pull_image() {
	...
	until
		(sudo docker login -u ${DOCKERHUBUSER} -p ${DOCKERHUBUSERPW} ${DOCKERHUB};
		 sudo docker pull ${DOCKERHUB}/hdpos46/${IMAGE}:${VERSION} | sudo tee /tmp/${IMAGE}-${VERSION}_pull.log;
		);do
		printf '.'
		sleep 1
	done
