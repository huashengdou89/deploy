...
WEBPORT=${WEBPORT:-18183}
PASOREPORTHTTPSPORT=${PASOREPORTHTTPSPORT:-18843}
PASOREPORTJMXPORT=${PASOREPORTJMXPORT:-19848}
...
start_pamysql() {
    ...
    ID=$(sudo docker run -d \
						 ...
						 ${DOCKERHUB}/component/${PREFIX}-mysql:5.6 \
...
start_paredis() {
    ...
    ID=$(sudo docker run -d \
						 ...
						 ${DOCKERHUB}/component/${PREFIX}-redis:2.8) >/dev/null 2>&1
}
...
start_container() {
    ...
    ID=$(sudo docker run -d \
						 -p ${WEBPORT}:8080 \
                         -p ${PASOREPORTHTTPSPORT}:8443 \
						 -p ${PASOREPORTJMXPORT}:${PASOREPORTJMXPORT} \
						 -e T_JVM_HOST=${INTRANETIP} \
						 -e T_JVM_PORT=${PASOREPORTJMXPORT} \
						 -e T_JAVA_OPTS="-XX:MaxPermSize=256m -Xms256m -Xmx8192m" \
						 -v ${LOGDIR}:/apache-tomcat/logs \
						 -v ${FILEDIR}:/opt/heading/Enclosure \
						 --restart=on-failure:3 \
						 --log-opt max-size=50m \
						 -m 2g \
						 --name ${IMAGE}_${ENVNAME} ${DOCKERHUB}/component/${IMAGE}:${VERSION}) >/dev/null 2>&1
}

deploying_container() {

    ID=$(sudo docker exec \
						-e CINTRANETIP="${INTRANETIP}" \
						-e CPAMYSQLPORT="${PAMYSQLPORT}" \
						-e CPAREDISPORT="${PAREDISPORT}" \
						${IMAGE}_${ENVNAME} sh -c '
											sed -i "
												s%\r$%%;\
												/Encryption/{s%#%%;s%md5%hdpos4%};\
												s%mysqlurl%${CINTRANETIP}%;\
												s%redisurl%${CINTRANETIP}%;\
												s%3306%${CPAMYSQLPORT}%;\
												s%6379%${CPAREDISPORT}%" /apache-tomcat/webapps/pasoreport-web/WEB-INF/classes/pasoreport-web.properties') >/dev/null 2>&1
}
